...
describe("V0.1 analysis flow", () => {
  ...
  it("uses the exact cautious zero-attention wording", async () => {
    ...
    choose();
    submit();
    expect(await screen.findByText("No Warning, High or Critical findings were produced by the current V0.1 rules.")).toBeInTheDocument();
  });
...
describe("Phase 9.2 analytical result semantics", () => {
  it("keeps observed completeness distinct from structural completeness", async () => {
    ...
    expect(quality.getByText("Observed dataset completeness")).toBeInTheDocument();
    expect(quality.getByText("83.4%")).toBeInTheDocument();
  ...
  it("renders profiling signals separately from Quality Engine findings", async () => {
    await renderResult();
    expect(screen.getByRole("heading", { name: "Structural profiling signals" })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Quality Engine findings" })).toBeInTheDocument();
  });
